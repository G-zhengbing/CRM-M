          ...
          <Col span="12">
            <FormItem label="课程类型" prop="type">
              <Select v-model="form.type" style="width:300px">
                <Option :value="i*1" v-for="(list,i) in course_type">{{list}}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="12" v-if="isUpdata">
...
export default {
  ...
  methods: {
    ...mapActions(["getTeacherList", "createdTeachers"]),
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.isUpdata) {
            ...
            formData.append("avatar", this.uploadList[0]?this.uploadList[0]:'');
    ...
    pageChange(num) {
      this.isLoading = true;
      this.getTeacherList({ ...this.seekForm }).then(res => {
        this.isLoading = false;
