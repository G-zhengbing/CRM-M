      ...
      <thead>
        <tr>
          <th
            ...
            :key="i"
            :class="{link:column.key == 'link_url'|| column.class == 'special'}"
          >
            ...
            <template v-else>
              <div class="handel" style="border-top:none;" v-if="column.title">{{column.title}}</div>
            ...
            <template v-else-if="column.type == 'action'">
              <template>
                <Btn
                  ...
                  :key="q"
                  :type="items.type"
                  @click="childer(item,items.type,i)"
                >{{items.text}}</Btn>
              </template>
            </template>
...
export default {
  ...
  methods: {
    ...
    childer(item, type,index) {
      if (type == "info") {
        this.childer1(item);
      } else if (type == "danger") {
        this.childer2(item);
      } else if (type == "link") {
        this.childer4(item,index);
    ...
    childer4(item,i) {
      this.$emit("childer4", item,i);
    },
    childer5(item) {
      this.$emit("childer5", item);
    },
    childer6(item) {
      this.$emit("childer6", item);
    },
    setSwitch(num) {
