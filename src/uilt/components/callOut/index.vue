<template>
  <div class="callOut">
    <!-- <span @click="callOut">呼出</span> -->
    <!-- <StudentsFollowUp ref="child" :test="test" :userIndex="userIndex" @test1="test1" /> -->
    <Modal
      v-model="modal5"
      title="学员跟进"
...
export default {
  ...
  methods: {
    ...mapMutations(["setFollowUp", "setUserId", "setOrder_id", "setRefresh"]),
    // 打电话
    async callOut() {
      // 打开跟进
      let res = await this.$request({
        ...
        url: CALLOUT,
        data: qs.stringify({
          mobile: this.row.mobile || this.row.accounts_mobile
        })
      });
      if (res.data.error) {
