          ...
          <Col span="12">
            ...
            >
              <Button
                ...
                :class="uploadData.chooseProduct?'uploaderrorbtn':''"
              >请选择固件</Button>
            </Upload>
          </Col>
          <Col span="12">
  ...
  data() {
    ...
    const validateUpload = (rule, value, callback) => {
      if (
        ...
        this.uploadData.uploadList.length === 0
      ) {
    ...
    return {
      ...
      ruleValidate: {
        ...
        // 上传的规则就是 自定义验证的规则
        uploadProduct: [{ validator: validateUpload, trigger: "change" }]
      }
    };
  },
  computed: {
    uploadDisable() {
      //根据数据判断上传按钮是否可以使用 非disable
      let obj = {
      ...
      if (!re.test(handlefname)) {
        ...
        bmf.md5(
          ...
          (err, md5) => {
            md5fname = md5;
            //把需要的参数传递给后端
            addFirmwareAction({ params })
              .then(res => {
                if (res.data) {
                  let obj = res.data;
                  //获取到文件上传地址赋值给data
                  this.uploadData.action =
                    location.protocol + "//" + obj["host"].split("://")[1];
